              ...
              <div className="space-y-2">
                ...
                {d.items.map((i) => (
                  <div key={i.id} className="flex items-center gap-3 border border-border rounded-lg p-3 bg-background">
                    ...
                    {canEdit && (
                      <button onClick={() => removeItem.mutate({ bookingId: b.id, itemId: i.id })} className="p-1 rounded hover:bg-destructive/10 text-destructive"><Trash2 className="w-4 h-4" /></button>
                    )}
                  </div>
                ))}
              </div>
              {canEdit && (
                <div className="border-t border-border pt-3 grid grid-cols-2 gap-2">
                  ...
                  <Button className="col-span-2 gap-1" onClick={async () => {
                    ...
                    try {
                      await addItem.mutateAsync({ bookingId: b.id, ...itemForm });
                  ...
                  <div key={m.id} className="flex items-center gap-3 border border-border rounded-lg p-3 bg-background">
                    ...
                    {canEdit && m.status !== "paid" && (
                      <Button size="sm" variant="outline" className="h-7 text-xs" onClick={async () => {
                        try {
                          await updatePayment.mutateAsync({ bookingId: b.id, pid: m.id, markPaid: true, method: "cash" });
                          toast({ title: "Marked paid" });
                        } catch (err) { toast({ title: "Failed", description: err instanceof Error ? err.message : "", variant: "destructive" }); }
                      }}>Mark paid</Button>
                    )}
                    {canEdit && m.status !== "paid" && (
                      <button onClick={() => removePayment.mutate({ bookingId: b.id, pid: m.id })} className="p-1 rounded hover:bg-destructive/10 text-destructive"><Trash2 className="w-4 h-4" /></button>
                    )}
                  </div>
                ))}
              </div>
              {canEdit && (
                <div className="border-t border-border pt-3 grid grid-cols-3 gap-2">
                  ...
                  <Button className="gap-1" onClick={async () => {
                    ...
                    try {
                      await addPayment.mutateAsync({ bookingId: b.id, label: payForm.label || "Milestone", dueDate: payForm.dueDate, amount: payForm.amount });
              ...
              <div className="space-y-2">
                ...
                {d.staff.map((s) => (
                  <div key={s.id} className="flex items-center gap-3 border border-border rounded-lg p-3 bg-background">
                    ...
                    {canEdit && (
                      <button onClick={() => removeStaff.mutate({ bookingId: b.id, aid: s.id })} className="p-1 rounded hover:bg-destructive/10 text-destructive"><Trash2 className="w-4 h-4" /></button>
                    )}
                  </div>
                ))}
              </div>
              {canEdit && (
                <div className="border-t border-border pt-3 grid grid-cols-2 gap-2">
                  ...
                  <Button className="col-span-2 gap-1" onClick={async () => {
                    ...
                    try {
                      await addStaff.mutateAsync({ bookingId: b.id, ...staffForm });
              ...
              <div className="space-y-2">
                ...
                {d.vendors.map((v) => (
                  <div key={v.id} className="flex items-center gap-3 border border-border rounded-lg p-3 bg-background">
                    ...
                    {canEdit && (
                      <button onClick={() => removeVendor.mutate({ bookingId: b.id, vid: v.id })} className="p-1 rounded hover:bg-destructive/10 text-destructive"><Trash2 className="w-4 h-4" /></button>
                    )}
                  </div>
                ))}
              </div>
              {canEdit && (
                <div className="border-t border-border pt-3 grid grid-cols-2 gap-2">
                  ...
                  <Button className="col-span-2 gap-1" onClick={async () => {
                    ...
                    try {
                      await addVendor.mutateAsync({ bookingId: b.id, ...vendorForm });
              ...
              <div className="space-y-1.5">
                ...
                {d.checklist.map((c) => (
                  <div key={c.id} className="flex items-center gap-3 border border-border rounded-lg p-2.5 bg-background">
                    <button onClick={() => toggleCheck.mutate({ bookingId: b.id, cid: c.id, completed: !c.completedAt })} className="text-muted-foreground hover:text-foreground">
                    ...
                    {canEdit && (
                      <button onClick={() => removeCheck.mutate({ bookingId: b.id, cid: c.id })} className="p-1 rounded hover:bg-destructive/10 text-destructive"><Trash2 className="w-4 h-4" /></button>
                    )}
                  </div>
                ))}
              </div>
                  ...
                  <Input placeholder="Checklist item..." value={checkForm.label} onChange={(e) => setCheckForm({ label: e.target.value })}
                    onKeyDown={async (e) => {
                      if (e.key === "Enter" && checkForm.label.trim()) {
                        try {
                          await addCheck.mutateAsync({ bookingId: b.id, label: checkForm.label.trim() });
                          setCheckForm({ label: "" });
                        } catch (err) { toast({ title: "Failed", description: err instanceof Error ? err.message : "", variant: "destructive" }); }
                      }
                    }} />
...
function MonthCalendar({
  ...
  const { data: bookings = [] } = useEventCalendar(fromIso, toIso);
    ...
    <Layout>
      ...
      {openDetailId !== null && (
        <BookingDetail
          ...
          onEdit={() => {
            const b = (list.data ?? []).find((x) => x.id === openDetailId);
            if (b) {
