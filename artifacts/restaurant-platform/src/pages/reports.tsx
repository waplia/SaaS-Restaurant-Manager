...
export default function ReportsPage() {
  ...
  const statCards = [
    { label: "Total Revenue", value: r ? `₹${Number(r.totalRevenue).toLocaleString()}` : "–", icon: DollarSign, color: "bg-orange-100 text-orange-600" },
    { label: "Total Orders", value: r?.totalOrders ?? "–", icon: ShoppingBag, color: "bg-blue-100 text-blue-600" },
    { label: "Avg Order Value", value: r ? `₹${Number(r.avgOrderValue).toFixed(0)}` : "–", icon: TrendingUp, color: "bg-green-100 text-green-600" },
    { label: "Total Tax Collected", value: r ? `₹${Number(r.totalTax).toFixed(0)}` : "–", icon: Receipt, color: "bg-purple-100 text-purple-600" },
  ];
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={r?.revenueByDay ?? []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              <defs>
              ...
              <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(d) => { try { return format(new Date(d), "MMM d"); } catch { return d; } }} />
        ...
        {/* Top items + orders chart */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="font-semibold text-foreground mb-4">Orders Per Day</h3>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={r?.revenueByDay ?? []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(d) => { try { return format(new Date(d), "MMM d"); } catch { return d; } }} />
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            {r?.topItems?.length > 0 ? (
              <BarChart width={300} height={200} data={r.topItems.slice(0, 5)} layout="vertical" margin={{ top: 0, right: 10, left: 80, bottom: 0 }}>
                <XAxis type="number" tick={{ fontSize: 11 }} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} width={80} />
                <Tooltip formatter={(v: number) => [v, "Orders"]} />
                <Bar dataKey="orderCount" radius={[0, 4, 4, 0]}>
                  {(r?.topItems ?? []).slice(0, 5).map((_: any, i: number) => (
