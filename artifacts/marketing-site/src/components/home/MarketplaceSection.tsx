        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {ITEMS.map(({ icon: Icon, title, desc }) => (
            <Link key={title} href="/integrations">
              <div className="group h-full p-3.5 md:p-5 rounded-xl border border-border bg-background hover:border-primary/40 hover:shadow-md transition-all">
                <div className="mb-2 md:mb-3 inline-flex p-2 rounded-lg bg-primary/10">
                  <Icon className="h-4 w-4 text-primary" />
                </div>
                <h3 className="font-semibold mb-1 md:mb-1.5 text-xs md:text-sm">{title}</h3>
                <p className="text-xs text-muted-foreground leading-relaxed mb-2 line-clamp-3 md:line-clamp-none">{desc}</p>
